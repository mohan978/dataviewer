...
async function fetchDerivativesCollectionDataOnly( collectionPath ) 
{
   ...
   snapshot.forEach(doc => {
	  ...
	  if ( doc.id.includes('part')==true )
	  {
		  
		  //console.log("fetchedData", fetchedData)
		  var jsonData = JSON.parse(fetchedData['data'])
		  //console.log("jsonData", jsonData)
		  for( var timeStamp in jsonData )
		  {
			  documents[timeStamp] = jsonData[timeStamp]
		  }
		  //console.log("documents", documents)
	  }
	  else if(fetchedData['data']!=undefined)
	  {
        ...
        if (await isObjectStoreAvailable(db)) {
          ...
          if (existingData && Object.keys(existingData).length>0 ) {
           ...
           console.log('Data fetched from the URL:');
			...
			//console.log("path.split('/')[3].replace(/\//g,'') ", path.split('/')[3].replace(/\//g,'') )
			//console.log("(hours > 15 || (hours === 15 && minutes > 45)) ", (hours > 15 || (hours === 15 && minutes > 45))  )
			if( path.split('/')[3].replace(/\//g,'') == currentDate && (hours > 15 || (hours === 15 && minutes > 45)) )
			//if( path.split('/')[3].replace(/\//g,'') == "12-Feb-2025" && (hours > 15 || (hours === 15 && minutes > 45)) )
			...
			else if( new Date( path.split('/')[3].replace(/\//g,'') ) < new Date(currentDate) )
			{
				// if Delivery_Data date is not equals to current date  
				// then save delivery data object even data is empty like this "{}"
				await saveDataToIDB(path, fetchedData, db);
			}
			else if( path.includes('/Delivery_Data/') ) // only for Delivery_Data
			{
